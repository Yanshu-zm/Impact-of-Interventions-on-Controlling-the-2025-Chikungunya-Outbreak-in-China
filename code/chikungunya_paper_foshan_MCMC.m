...
FOSHAN_INDEX = 4;
disp(cities(FOSHAN_INDEX))
SHENZHEN_INDEX = find(cities == "广东省深圳市");

%流量比，但这是2个数据，旧数据不可靠，使用新数据
disp(sum(FlowNet(FOSHAN_INDEX,22:end,:),'all')/7)
disp(sum(FlowNet(FOSHAN_INDEX,SHENZHEN_INDEX,:),'all')/7)
disp(sum(FlowNet(FOSHAN_INDEX,22:end,:),'all')/7/32000)

num_other_cites = ncites-21; % 广东21省市在前面
E_import_nums = zeros( num_other_cites, TIMELENGHT, num_iteration); % 佛山源头，目的地，时间
AI_import_nums = zeros( num_other_cites, TIMELENGHT, num_iteration); % 佛山源头，目的地，时间

E_import_nums_sz = zeros(  TIMELENGHT, num_iteration);
AI_import_nums_sz = zeros( TIMELENGHT, num_iteration); 

%% Calibrate with Shenzhen
for t = 9 : TIMELENGHT
   wd = weekday(datetime(2025,7,0)+caldays(t));
   % flow = FlowNet(FOSHAN_INDEX,SHENZHEN_INDEX,wd);
   flow = 25000; %旧数据不可靠，使用新数据
   % cal the number of outputs
   prevalance = ModelingOutputs(t-1,5,:)./ps_foshan; % 改用E
   prevalance_asymI = (1-param.sym_ratio) * ModelingOutputs(t-1,6,:)./ps_foshan;
   E_import_nums_sz(t,:) = squeeze(flow)' * squeeze(prevalance)' ;
   AI_import_nums_sz(t,:) = squeeze(flow)' * squeeze(prevalance_asymI)';
end
sum(mean(E_import_nums_sz,2),1) %result: 3.2W 141.7212
...
for t = 9:TIMELENGHT
    wd = weekday(datetime(2025,7,0)+caldays(t));
    flow = FlowNet(FOSHAN_INDEX,22:end,wd);
    prevalance     = ModelingOutputs_noIntv(t-1,5,:)./ps_foshan;
    prevalance_asy = (1-param.sym_ratio)*ModelingOutputs_noIntv(t-1,6,:)./ps_foshan;
...
for t = 9 : TIMELENGHT
   ...
   % one week +
   prevalance = ModelingOutputs_owh(t-1,5,:)./ps_foshan; % 改用E
   prevalance_asymI = (1-param.sym_ratio) * ModelingOutputs_owh(t-1,6,:)./ps_foshan;
   ...
   % two week +
   prevalance = ModelingOutputs_twh(t-1,5,:)./ps_foshan; % 改用E
   prevalance_asymI = (1-param.sym_ratio) * ModelingOutputs_twh(t-1,6,:)./ps_foshan;
   ...
   % s1.5
   prevalance = ModelingOutputs_s15(t-1,5,:)./ps_foshan; % 改用E
   prevalance_asymI = (1-param.sym_ratio) * ModelingOutputs_s15(t-1,6,:)./ps_foshan;
   ...
   % s1.5 + one week
   prevalance = ModelingOutputs_s15_owh(t-1,5,:)./ps_foshan; % 改用E
   prevalance_asymI = (1-param.sym_ratio) * ModelingOutputs_s15_owh(t-1,6,:)./ps_foshan;
...
for t = 9 : TIMELENGHT
   ...
   flow = FlowNet(FOSHAN_INDEX,22:end,wd);
    ...
    % ---- 延迟 1 周 ----
   prevalance = ModelingOutputs_l1w(t-1,5,:)./ps_foshan;
   prevalance_asymI = (1-param.sym_ratio)*ModelingOutputs_l1w(t-1,6,:)./ps_foshan;
   ...
   AI_import_nums_l1w(:,t,:) = immigrant_risk_ratio_simulation * squeeze(flow)' * squeeze(prevalance_asymI)';
    ...
    % ---- 延迟 2 周 ----
   prevalance = ModelingOutputs_l2w(t-1,5,:)./ps_foshan;
   prevalance_asymI = (1-param.sym_ratio)* ModelingOutputs_l2w(t-1,6,:)./ps_foshan;
...
for t = 9 : TIMELENGHT
   ...
   flow = FlowNet(FOSHAN_INDEX,22:end,wd);
    % ---- 强度 0.5 ----
   prevalance = ModelingOutputs_s05(t-1,5,:)./ps_foshan;
   prevalance_asymI = (1-param.sym_ratio)*ModelingOutputs_s05(t-1,6,:)./ps_foshan;
...
for t = 9 : TIMELENGHT
   ...
   flow = FlowNet(FOSHAN_INDEX,22:end,wd);
    % ---- 强度 0.5 + 提前 1 周 ----
   prevalance = ModelingOutputs_s05_owh(t-1,5,:)./ps_foshan;
   prevalance_asymI = (1-param.sym_ratio)*ModelingOutputs_s05_owh(t-1,6,:)./ps_foshan;
...
fn = sprintf('result/result_foshan/CrossCity_ge1_%s.xlsx', datestr(now,'yyyymmdd_HHMMSS'));
...
title('Cities with cumulative ≥1 case (mean ± 95% CI)');
