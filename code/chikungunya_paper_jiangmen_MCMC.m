...
% 江门蚊虫数据
vector_dataTable = readtable("data/江门蚊媒数据.csv", ...
...
vec_startDate = datetime(2025, 9, 14);
deltas_BI = compute_delta(BI_Row, 0.373, 3);
deltas_BI = [[nan,nan,nan] ,deltas_BI', [nan,nan,nan]];
...
if strcmp(source_city, '江门')
    daily_case = readtable('data/jiangmen_Daily_cases.xlsx',VariableNamingRule='preserve');
...
if (use_mc)
    ...
    out = mcmcpred(res,chain(burned_in:end,:),[],data,@f_model,num_iteration); %
...
%% Intervention strength get 1.5x & Intervention one week early
 ...
 %% 新增：0.5 倍强度 + 提前一周
    ...
    param_noIntv.carrying_capacity_reduced_rate = 1;
    param_noIntv.ip_reduce = 1;
    param_noIntv.carrying_capacity_decline_begin = 999;
    param_noIntv.quarantine_start = 999;
